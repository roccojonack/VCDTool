                                 ...
                                 std::vector<std::string> filterVector)
{
    ...
    if (fullpath)
    {
        if (foundScope)
        {
            ...
            {
                print_scope_signals(trace, scope, local_parent, m_root);
            }
        }
        else
        {
...
void VCDAnalyzer::print_stat_signals(VCDFile *trace, VCDScope *scope, std::string local_parent)
{
    ...
    { // iterating over all signals in current scope
        ...
        if (trace->get_signal_values(signal->hash)->size() < 2)
            continue; // print only values which toggle at least one time
        Json::Value json_tmp;
        json_tmp["name"] = signal->reference;
...
void clean_signal_names(std::string file_name, std::string out_file_name)
{
    ...
    if (ifs.is_open())
    {
        ...
        while (std::getline(ifs, myString))
        {
            ...
            { // replace real value with bitvector
                std::string tmp1 = m[1];
                std::string tmp2 = m[2];
