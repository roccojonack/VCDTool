            ...
            if value['type']['name']=='wire':
                if len(value['data'])>1:
                            ...
                            for name in globals.design_info['interfaces']:
                                if name['agent'] == unit_name:
                                    ...
                                    if direction =="in" and type == "AXI4":
                                        ...
                                        stat_type = globals.ncore_stats['IOAIU'][stat_name]
                                        unit_type = "IOAIU"
                                    if direction =="out" and type == "AXI4":
                                        logger.debug("found DMI in designinfo ")
                                        stat_type = globals.ncore_stats['DMI'][stat_name]
                                        unit_type = "DMI"

                                    if direction =="in" and type == "CHI":
                                        logger.debug("found CAIU in designinfo ")
                                        stat_type = globals.ncore_stats['CAIU'][stat_name]
                                        unit_type = "CAIU"
                    ...
                    for i in value['data']:
                        ...
                        if stat_type == "level":
                            if val in hist:
                                hist[val] += (currentTime-previousTime)
                            else:
                                hist[val] = (currentTime-previousTime)
                        res.append(val)
                    ...
                    if legato:
                        ...
                        if unit_name in globals.result:
                            globals.result[unit_name].append(res)
                        else:
                            globals.result[unit_name] = [] 
                    # logger.info("    ", scope, value['name'], stat_name, stat_type, value['type']['width'], len(value['data']), min_value, avg, max_value, value['data'][0][0], value['data'][-1][0])
